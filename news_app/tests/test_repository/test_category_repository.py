...
def test_get_all_categories(repo):
    with patch(
        "server.repository.category_repository.rows_to_models",
...
def test_get_category_by_name(repo):
    with patch(
        "server.repository.category_repository.row_to_model", return_value=MagicMock()
    ):
        conn = repo.db.connect.return_value
        cursor = MagicMock()
        conn.close = MagicMock()
...
def test_add_category_duplicate(repo):
    with patch("server.utils.repository_helper.with_cursor") as mock_cursor, patch(
        "server.utils.repository_helper.safe_execute",
        side_effect=lambda f, default=None: f(),
    ):
        cursor = MagicMock()

...
def test_get_user_by_id(repo):
    ...
    with patch.object(UserRepository, "db", db, create=True), patch(
        "server.utils.repository_helper.row_to_model", return_value=user_instance
    ):
...
def test_get_user_keywords_json(repo):
    ...
    with patch("server.utils.repository_helper.with_cursor", return_value=cursor):
        cursor.__enter__.return_value = cursor
        cursor.fetchone.return_value = {"keywords": '["abc", "def"]'}

        with patch("json.loads", return_value=["abc", "def"]):

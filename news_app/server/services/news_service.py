...
from server.utils.service_helper import parse_ts, get_category_id, get_current_utc_time
...
class NewsService:
    ...
    def _insert_from_provider(
        self, client, source, categories: List[str], total_to_fetch: int
    ) -> int:
        inserted = 0
    ...
    def get_latest_articles(
        self, category_name: Optional[str] = None, limit: int = 20, offset: int = 0
    ) -> List[Article]:
        cat_id = None
        if category_name:

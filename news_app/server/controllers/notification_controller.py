...
class NotificationPreferences(Resource):
    ...
    def post(self):
        ...
        if keywords_csv is None or keywords_csv.strip() == "":
            ...
            user_keys = [
                k.strip() for k in keywords_csv.split(",") if k.strip()
            ]
        ...
        valid_set = {
            cat.name.lower() for cat in category_repo.get_all_categories()
        }
        ...
        encoded_blob = encode_preferences(
            cleaned_categories_csv, cleaned_keywords_csv
        )
